    ...
    <>
              ...
              <nav className="space-y-1 px-4">
                {links.map((link) => (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={`flex items-center px-3 py-2 text-sm font-medium gap-3 text-black${
                      isActive(link.path)
      ...
      <div className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 bg-[#2B4F4B]">
        <div className="flex-1 flex flex-col pt-5">
          ...
          <nav className="flex-1 px-2">
            {links.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`flex items-center px-3 py-2 text-sm font-medium gap-3 text-white ${
                  isActive(link.path)
                  ? "border-l-2 border-white font-bold"
          ...
          <div className="sticky top-0  flex h-16 justify-center">
            <Menu as="div" className="relative inline-block text-left">
            <MenuButton >
            <span className="flex min-w-full p-3 items-center gap-2 hover:border-l-2 border-white">
                <span>
                  <img className="h-10 w-10 rounded-full object-cover "
                  src="https://randomuser.me/api/portraits/women/68.jpg" // or your uploaded avatar image
                  alt="User Avatar"
                  />
                </span>
